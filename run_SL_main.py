...
from baselines.RMTL.train.run import get_model, sltrain
...
def get_SL_args():
    ...
    parser.add_argument('--message', type=str, default='SL')
    # ###########################################################################
...
def main(task_num, expert_num, model_name, epoch, learning_rate, batch_size, embed_dim, weight_decay, device, args):
    ...
    for epoch_i in range(epoch + 1):
        if epoch_i > 0:
            train_loss = sltrain(model, optimizer, train_data_loader, criterion, device)
    ...
    save_path = os.path.join(save_dir, "chkpt", f"rt_{args.model_name}_0.0.pt")
