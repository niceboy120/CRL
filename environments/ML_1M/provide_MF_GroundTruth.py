...
def load_data():
    ratings_df = pd.read_csv(os.path.join(DATAPATH, "ratings.dat"), delimiter='::', header=None,
...
def train_and_evaluate(model, train_loader, test_loader, criterion, optimizer, epochs=5, device="cpu"):
    for epoch in range(epochs):
        ...
        for users, movies, ratings in tqdm(train_loader, desc=f'Epoch {epoch + 1} - Training'):
            ...
            total_train_mae += torch.mean(torch.abs(predictions - ratings)).item()

        average_train_loss = total_train_loss / len(train_loader)
        average_train_mae = total_train_mae / len(train_loader)
        ...
        print(f"Epoch {epoch + 1}, "
              f"Training Loss (MSE): {average_train_loss:.4f}, "
              f"Test Loss (MSE): {average_test_loss:.4f}, "
              f"Train MAE: {average_train_mae:.4f}, "
              f"Test MAE: {average_test_mae:.4f}")
