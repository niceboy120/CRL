...
DATAPATH = os.path.join(CODEPATH, "rt")
...
class RetailRocketRLDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_path):
        ...
        self.features = self.get_features(os.path.join(DATAPATH, "item_feadf.csv"), self.cate_cols)
        # mdp_data = pd.read_csv(dataset_path, usecols=["visitorid", 'itemid', 'click', 'pay'],engine='python',error_bad_lines=False)
        mdp_data = pd.read_csv(dataset_path, usecols=["visitorid", 'itemid', 'click', 'pay'], engine='python')
        data = mdp_data.merge(self.features, on="itemid", how='left')
        data.fillna(0)
        self.categorical_data = data[self.cate_cols].values.astype(np.int64)
        ...
        self.session_id = mdp_data['visitorid'].values.astype(np.int64)
